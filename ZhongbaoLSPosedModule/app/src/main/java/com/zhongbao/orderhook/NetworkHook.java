...
public class NetworkHook {
    ...
    // 订单筛选器（可通过 MainActivity 配置）
    private static volatile OrderFilter orderFilter;

    public static void setOrderFilter(OrderFilter filter) {
        orderFilter = filter;
        MainHook.logI("订单筛选规则已更新");
    }

    public static OrderFilter getOrderFilter() {
        if (orderFilter == null) {
            orderFilter = OrderFilter.getDefault();
        }
        return orderFilter;
    }

    // ============================================================
    // 订单捕获统一回调（日志 + 保存 + 筛选 + 推送）
    // ============================================================
    private static void onOrderCaptured(String tag, String url, String json) {
        ...
        boolean shouldGrab = filter.shouldGrab(json);
        MainHook.logI("  筛选结果: " + (shouldGrab ? "✓ 命中 → 触发抢单" : "✗ 不命中"));

        // 3) 推送到所有通道
        Broadcaster.getInstance().broadcastOrder(tag, url, json);

        // 4) 如果命中筛选，执行抢单
        if (shouldGrab) {
            triggerGrabOrder(tag, url, json);
            // === 手机端本地抢单 (无需电脑) ===
            try {
                GrabExecutor executor = GrabExecutor.getInstance();
                if (executor.isEnabled()) {
                    String orderId = extractOrderId(json);
                    executor.executeGrab(orderId);
                    MainHook.logI("【本地抢单】已触发: " + orderId);
                }
            } catch (Throwable t) {
                MainHook.logE("本地抢单触发异常", t);
            }
        }
    ...
    /**
     * 触发抢单信号：通过 Broadcast + HTTP 两种方式通知外部
     */
    private static void triggerGrabOrder(String tag, String url, String json) {
        try {
            // 构造抢单 payload
            JSONObject grabPayload = new JSONObject();
            grabPayload.put("action", "GRAB_ORDER");
            grabPayload.put("tag", tag);
            grabPayload.put("url", url);
            grabPayload.put("order", new JSONObject(json));
            grabPayload.put("timestamp", System.currentTimeMillis());
            String payload = grabPayload.toString();

            MainHook.logI("【触发抢单】Payload: " + payload.substring(0, Math.min(payload.length(), 500)));

            // 通过 Broadcaster 推送到所有通道
            Broadcaster.getInstance().broadcastOrder("GRAB_ORDER_" + tag, url, payload);

            // 特别的 Broadcast: 单独发送抢单动作
            android.content.Intent intent = new android.content.Intent("com.zhongbao.orderhook.GRAB_ORDER");
            intent.putExtra("tag", tag);
            intent.putExtra("url", url);
            intent.putExtra("json", json);
            // 注意: 这里没有 Context，无法直接 sendBroadcast
            // Broadcaster 在有 Context 的情况下会自动补发
        } catch (Throwable t) {
            MainHook.logE("触发抢单异常", t);
        }
    }
    ...
    private static String extractOrderId(String json) {
        try {
            ...
            if (obj.has("waybill_id")) return obj.getString("waybill_id");
            if (obj.has("order_id")) return obj.getString("order_id");
            if (obj.has("data")) {
